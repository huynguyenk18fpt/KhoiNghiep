...
type PaymentMethodsProps = {
  ...
  onChange: (value: CheckoutInfo["paymentMethod"]) => void;
};
...
export default function PaymentMethods({ value, onChange }: PaymentMethodsProps) {
  return (
    <div>
      ...
      <RadioGroup value={value} onValueChange={(nextValue) => onChange(nextValue as CheckoutInfo["paymentMethod"])}>
        ...
        <div className="flex items-start gap-3 p-4 rounded-lg border border-gray-200 hover:border-blue-500 transition-colors mb-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500 mt-1 ml-7">Demo FE-only chỉ ghi nhận lựa chọn, chưa xử lý thanh toán thật.</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 rounded-lg border border-gray-200 hover:border-blue-500 transition-colors">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500 mt-1 ml-7">Demo FE-only chỉ ghi nhận lựa chọn, chưa tích hợp cổng thanh toán.</p>
          </div>
        </div>
      </RadioGroup>
    </div>

...
export default function WeeklyTimeline({
  ...
  const weekly =
    plan?.weeklyPlan && Array.isArray(plan.weeklyPlan) && plan.weeklyPlan.length
      ? plan.weeklyPlan
      : Array.from({ length: 8 }).map((_, i) => ({
          ...
          pool: [
            {
              name: "Làm quen nước & thở",
              durationMin: 25,
              drills: ["thở ra dưới nước 3×10", "trượt nước 6–8 lần"],
            },
            {
              name: "Nổi sấp/ngửa",
              durationMin: 20,
              drills: ["nổi sao biển 3×20s", "đổi tư thế sấp-ngửa"],
            },
          ],
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {w.sessionsPerWeek || 3} buổi/tuần • ~{Math.ceil(minutes / 60)}h/tuần
                    </p>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {w.pool.map((p: any, idx: number) => (
                          <div
                            key={idx}
                            className="p-3 rounded-lg bg-sky-50 border-l-4"
